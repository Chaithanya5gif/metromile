...
| **Payments** | Auto-processed on trip completion — no manual checkout |
...
```
Login → Select origin & destination → Choose departure time
  → Fare calculated → Confirm booking
  → [PENDING] Waiting for driver
  → [CONFIRMED] Driver accepted → View driver details
  → Live tracking during ride
  → Arrive → Auto payment → Rate driver ✓
```

### Driver flow

```
Login → Set status "Available"
...
| Max passengers per auto | 3  |
| Max passengers per car | 4 |
